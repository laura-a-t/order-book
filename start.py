...
def parse_messages():
    order_book_depth = int(sys.argv[1])
    logger = Logger()
    order_book = OrderBook()
    parser = Parser()
    processor = Processor(Orders(), order_book)
    stream = sys.stdin.buffer
    while sequence_no_bytes := stream.read(4):
        sequence_no = int.from_bytes(sequence_no_bytes, LITTLE_ENDIAN)
        message_size = int.from_bytes(stream.read(4), LITTLE_ENDIAN)
        message = parser.parse_message(BytesIO(stream.read(message_size)))
